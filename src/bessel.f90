...
!     a nicety that you can omit if five figure accuracy is good enough
    ...
    subroutine besIExp ( gamma_re, gamma_im, lMax, &
        expBes_re, expBes_im, &
        expBesPrime_re, expBesPrime_im, &
        expBesOverGam_re, expBesOverGam_im )
        ...
        real(kind=dbl), intent(in) :: gamma_re, gamma_im
        real(kind=dbl) :: expBes_re(lMax+1), expBesPrime_re(lMax+1), expBesovergam_re(lMax+1)
        real(kind=dbl) :: expBes_im(lMax+1), expBesPrime_im(lMax+1), expBesovergam_im(lMax+1)
        complex(kind=dbl) :: expBes(lMax+1), expBesPrime(lMax+1), expBesovergam(lMax+1)
        ...
        do l = 0, lmax
           ...
           expBes_re(l+1) =  real(expBes(l+1))
           expBes_im(l+1) =  aimag(expBes(l+1))

           expBesPrime_re(l+1) =  real(expBesPrime(l+1))
           expBesPrime_im(l+1) =  aimag(expBesPrime(l+1))

           expBesOverGam_re(l+1) = real(expBesOverGam(l+1))
           expBesOverGam_im(l+1) = aimag(expBesOverGam(l+1))
